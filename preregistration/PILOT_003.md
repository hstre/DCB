# PILOT_003 preregistration — FROZEN PRE-DATA

**Status:** frozen before any PILOT_003 target API call. The non-target provider contract check and offline mechanical checks passed before this freeze.
...
Use exactly the 20 task/probe texts in `prompts/item_bank_pilot_002.json`, according to the checked mapping in `prompts/seed_map_pilot_003.json`: P002-001→P003-001 through P002-020→P003-020. `target_consideration` is not used by P003.
...
## Freeze rule
Frozen together before the first target call: this preregistration; `protocol/PILOT_003_PROVENANCE.md`; `prompts/PILOT_003_TEMPLATES.md`; `prompts/seed_map_pilot_003.json`; `runner/run_pilot_003.py`; `scoring/schema_pilot_003.json`; `analysis/pilot_003_analysis.py`; `.github/workflows/run-pilot-003.yml`; and `protocol/FREEZE_PILOT_003.md`.

After the first P003 target API call, any change to the frozen experimental components becomes PILOT_004.
