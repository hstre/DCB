"""Frozen deterministic analysis for DCB PILOT_003."""
...
def integrity(records):
    ...
    for rec, call in calls:
        ...
        if missing:
            failures.append({"seed_id": rec["seed_id"], "attempt": rec.get("attempt"), "arm": call.get("arm"), "failed": missing})
    return {"n_logged_calls": len(calls), "all_required_true": bool(calls) and not failures, "failures": failures}


def summarize(all_records):
    first = earliest_attempts(all_records)
    included = [r for r in first if not r.get("excluded")]
    first_ids = {(r["seed_id"], r.get("attempt", 1)) for r in first}
    later = [r for r in all_records if (r["seed_id"], r.get("attempt", 1)) not in first_ids]
    exclusion_counts = {}
    for rec in first:
        if rec.get("excluded"):
...
if __name__ == "__main__":
    ...
    if not records:
        raise SystemExit("No PILOT_003 attempt records found.")
