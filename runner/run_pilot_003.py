#!/usr/bin/env python3
"""Frozen execution harness for DCB PILOT_003."""
...
SEED_MAP = ROOT / "prompts" / "seed_map_pilot_003.json"
...
def expected_tool_result(
    registry: dict[str, dict[str, Any]], bundle_id: str, current_subject_id: str,
    *, unspecified: bool, control_code: str | None,
) -> dict[str, Any]:
...
def tool_call(
    prompt: str, *, registry: dict[str, dict[str, Any]], bundle_id: str,
    current_subject_id: str, unspecified: bool, control_code: str | None,
    model: str, base: str, key: str,
    ...
    expected = expected_tool_result(registry, bundle_id, current_subject_id, unspecified=unspecified, control_code=control_code)
...
def run(seed_id: str, attempt: int) -> dict[str, Any]:
    ...
    rec: dict[str, Any] = {
        "pilot": "PILOT_003", "status": "FROZEN_EXECUTION", "seed_id": seed_id,
        "attempt": attempt, "source_seed": seed["source_seed"], "family": seed["family"],
        "model": model, "model_version": version, "api_base": base,
        "request_parameters": {"temperature": 0.0, "thinking": THINKING},
        "interface_level": "I1_SIM", "s1_status": "NOT_TESTABLE",
        "registry_version": REGISTRY_VERSION, "excluded": False,
        "exclusion_code": None, "exclusion_detail": None, "errors": [],
        "primary": {}, "placebo": {}, "label_controls": {}, "conflict": {},
        "calibration": {}, "tool_integrity": [], "started_unix": time.time(),
    }
    ...
    try:
        ...
        def do_tool(prompt: str, current_subject_id: str, *, unspecified: bool,
                    control_code: str | None, arm: str, family: str) -> dict[str, Any]:
            try:
                text, integ = tool_call(
                    prompt, registry=registry, bundle_id=bundle_id,
                    current_subject_id=current_subject_id, unspecified=unspecified,
                    control_code=control_code, model=model, base=base, key=key,
                )
        ...
        for arm in PRIMARY:
            ...
            for fam in ("related", "unrelated"):
                rec["primary"][arm][fam] = do_tool(
                    tool_prompt(artifact, r, bundle_id, seed[fam]), current,
                    unspecified=False, control_code=None, arm=arm, family=fam,
                )
        ...
        for arm in PLACEBO:
            ...
            rec["placebo"][arm] = do_tool(
                tool_prompt(artifact, r, bundle_id, seed["related"]), f"placebo::{seed_id}",
                unspecified=True, control_code=PLACEBO_CODES[code_key], arm=arm, family="related",
            )

        for arm in LABEL:
            result = scored(plain_call([
        ...
        for name, claim_own, current in (
            ...
            rec["conflict"][name] = do_tool(
                conflict_prompt(claim_own, artifact, canonical_r, bundle_id, seed["related"]), current,
                unspecified=False, control_code=None, arm=name, family="related",
            )
        ...
        if seed_id in CAL:
            for arm in PRIMARY:
                ...
                rec["calibration"][arm] = do_tool(
                    tool_prompt(artifact, r, bundle_id, seed["related"]), current,
                    unspecified=False, control_code=None, arm="CAL:" + arm, family="related",
                )
